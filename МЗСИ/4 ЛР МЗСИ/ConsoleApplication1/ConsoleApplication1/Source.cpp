...
// Функция шифрования
string encryption(string &vector)
{
    //Начальную перестановка
    string res = "";
    for (int i = 0; i < 64; i++)
    {
        res += encrStr[firstPermutTabText[i] - 1];
    }
    // Деление результата на две равные половины
    string left = res.substr(0, 32);
    string right = res.substr(32, 32);
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        for (int i = 0; i < 48; i++)
        {
            rightExpand += right[expandTab[i] - 1];
        };
        // Применяем операцию "Исключающее ИЛИ"
        string strXOR = XOR(roundKeys[i], rightExpand);
        string res = "";
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            string rowStr = strXOR.substr(i * 6, 1) + strXOR.substr(i * 6 + 5, 1);
            int row = binToDec(rowStr);
            string colStr = strXOR.substr(i * 6 + 1, 1) + strXOR.substr(i * 6 + 2, 1) + strXOR.substr(i * 6 + 3, 1) + strXOR.substr(i * 6 + 4, 1);
            int col = binToDec(colStr);
        ...
        if (count == 0)
        {
            result = XOR(res, vector);
            previousEncryp = result;
        }
        if (count > 0) //с предыдущими блоками
        {
            result = XOR(res, previousEncryp);
            previousEncryp = "";
            previousEncryp = result;
        }
...
void coding()
{
    ...
    string str;
    getline(cin, str);
    cout << "Введите ключ, состоящий из 8 символов: ";
    string key;
    getline(cin, key);
    cout << "Введите вектор, состоящий из 8 цифр: ";
    string IV;
    getline(cin, IV);

    key = strToBin(key);
    int symbols = 0;
    //если исходный текст не делится на блоки по 64 бита, то он 
     //дополняется символами с конца
    for (int i = 0; i < (str.length() % 8); i++)
    {
        str += " ";
        symbols++;
    }
    string encrypted = "";  //зашифрованное сообщение
    int count = 0;
    string textBlock;       //результат шифрования блока текста	//шифрование сообщения
    for (int i = 0; i < (str.length() / 8); i++)
    {
        string block = "";
        for (int j = 0; j < 8; j++)
        ...
        genKeys(key);
        textBlock = encryption(IV);
        encrypted += textBlock;
    }
    cout << endl;
    cout << "Зашифрованное сообщение: " << encrypted << endl
        << endl;
    string encryptedStr = binToStr(encrypted);
    cout << "Зашифрованное сообщение: " << encryptedStr << endl
        << endl;
    ...
    count = 0;
    string decrypted;  //расшифрованное сообщение 
    ...
    for (int k = 0; k < (str.length() / 8); k++)
    {
        ...
        for (int m = 0; m < 64; m++)
        {
            binBlock += encrypted[count];
            count++;
        }
        encrStr = binBlock;
        //при расшифровке данных все действия выполняются в 
         //обратном порядке 
        string result = encryption(IV);
        decrypted += result;
    }
    cout << "Расшифрованное сообщение: " << endl << decrypted
        << endl << endl;
